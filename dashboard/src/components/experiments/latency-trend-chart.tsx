    ...
    <div className="bg-popover border border-border rounded-lg shadow-lg px-3 py-2 text-sm">
      ...
      {payload.map((entry, index) => (
        <div key={index} className="flex items-center gap-2">
          ...
          <span className="font-medium">{entry.value.toFixed(0)}ms</span>
        </div>
      ))}
    </div>
  )
...
export function LatencyTrendChart({ timeseries, loading }: LatencyTrendChartProps) {
  // Group data by bucket - useMemo ensures re-computation when timeseries changes
  const chartData = useMemo(() => {
    ...
    timeseries.forEach((point) => {
      ...
      bucket[point.variant_name] = point.avg_latency_ms
    })
    ...
    return (
      <div className="bg-card border rounded-xl shadow-sm overflow-hidden">
        <div className="px-4 py-3 border-b flex items-center gap-2">
          ...
          <span className="text-sm font-semibold">Latency Over Time</span>
        </div>
