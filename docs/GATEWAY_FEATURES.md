...
| External User Attribution | ✅ Complete | customer/user ID for SaaS builders |
| Token Metadata | ✅ Complete | Flexible JSONB for custom attributes |
| Bulk Token Operations | ✅ Complete | Create/revoke up to 500 tokens |

**Database Schema** (from migrations):
- `tokens` table with 40+ columns
- Policy binding via `token_policy_bindings`
- Spend caps via `token_spend_caps`
- `external_user_id` for SaaS builder attribution (migration 043)
- `metadata` JSONB for flexible custom data (migration 043)

### 2.2 SaaS Builder Support

**Implementation Status**: ✅ Fully Implemented

TrueFlow supports two primary use cases for virtual tokens:
1. **Enterprise**: Give tokens to internal agents (traditional use case)
2. **SaaS Builders**: Give tokens to their customers (attribution support)

**Key Features**:
| Feature | Description |
|---------|-------------|
| `external_user_id` | Customer identifier from your billing system |
| `metadata` JSONB | Flexible storage for plan tier, region, custom attributes |
| Bulk Creation | Create up to 500 tokens per API call |
| Bulk Revocation | Revoke tokens by external_user_id, team_id, or token IDs |
| User Analytics | Track spend per customer across all requests |
...
curl -X POST http://localhost:8443/api/v1/tokens \
  ...
  -d '{
    ...
    "external_user_id": "cust_acme_123",
...
```bash
curl -X POST http://localhost:8443/api/v1/tokens/bulk \
  -H "Authorization: Bearer $ADMIN_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "tokens": [
      {"name": "customer-1", "upstream_url": "...", "external_user_id": "cust_1"},
      {"name": "customer-2", "upstream_url": "...", "external_user_id": "cust_2"}
    ]
  }'
```
...
- `external_user_id` attribution for SaaS builders (migration 043)

### 10.4 Analytics & Spend Tracking

**Implementation Status**: ✅ Fully Implemented

**Spend Breakdown Grouping Options**:
| Group By | Description |
|----------|-------------|
| `model` | Spend per LLM model (gpt-4o, claude-3, etc.) |
| `token` | Spend per virtual token (agent key) |
| `tag:KEY` | Spend per custom tag value from X-Properties header |
| `external_user_id` | Spend per customer/user (SaaS builders) |
| `team` | Spend per team |

**API Endpoints**:
- `GET /api/v1/analytics/spend/breakdown?group_by=external_user_id`
- `GET /api/v1/analytics/users` - User-level spend summary with token counts

**Example User Analytics**:
```bash
curl "http://localhost:8443/api/v1/analytics/users?hours=720" \
  -H "Authorization: Bearer $ADMIN_KEY"
```
...
**Tables** (43 migrations):
1. `organizations` - Multi-tenant root
2. `credentials` - Encrypted API keys
3. `tokens` - Virtual tokens (with external_user_id, metadata since migration 043)
4. `policies` - Policy definitions
5. `policy_versions` - Version history
6. `audit_logs` - Request/response logs (with external_user_id since migration 043)
...
**Migration Files**: 43 migrations (001-043)
...
**Implementation Status**: ✅ 85+ Endpoints Implemented

**Resource Groups**:
- `/api/v1/tokens` - Token CRUD (includes bulk operations)
- `/api/v1/tokens/bulk` - Bulk token creation (up to 500)
- `/api/v1/tokens/bulk-revoke` - Bulk token revocation by filter
...
- `/api/v1/analytics/users` - User-level spend analytics (SaaS builders)
...
### 18.2 Token API Reference

**Token Filtering** (GET /api/v1/tokens):
| Query Parameter | Description |
|-----------------|-------------|
| `external_user_id` | Filter by customer/user ID |
...
| `upstream_url` | string | LLM provider URL |
| `credential_id` | UUID | Linked credential |
| `external_user_id` | string | Customer ID for SaaS builders |
| `metadata` | object | Custom JSONB attributes |
| `team_id` | UUID | Team assignment |
| `tags` | object | Cost attribution tags |
| `policy_ids` | array | Policy bindings |
| `spend_cap_daily` | number | Daily spend limit |
...
| Observability | 100% | OTLP, Prometheus, audit logs, user analytics |
| Cost Management | 100% | Spend caps, budget alerts, per-customer tracking |
| API Surface | 100% | 85+ REST endpoints including bulk operations |
| Background Jobs | 100% | 6 scheduled tasks |
| CLI | 100% | Token, credential, policy, IaC |
| SaaS Builder | 100% | External user attribution, bulk ops, user analytics |
